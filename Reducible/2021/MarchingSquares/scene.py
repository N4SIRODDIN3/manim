...
class MarchingSquare:
    ...
    def get_polygon_for_case(self, case, implicit_function, value=1, gradient=False):
        ...
        return self.get_polygon(
            polygon_identifier, implicit_function, value=value, gradient=gradient
        )
    ...
    def get_polygon(self, identifier, implicit_function, value=1, gradient=False):
        ...
        for point in identifier:
            if isinstance(point, int):
                ...
                edge = point
                point_on_edge = self.get_point_on_edge(
                    edge, implicit_function, value=value
                )
    ...
    def get_point_on_edge(self, edge, implicit_function, value=1):
        ...
        value_u_pos = get_func_val_from_map(
            self.function_map, (u_pos[0], u_pos[1]), implicit_function
        )
        value_v_pos = get_func_val_from_map(
            self.function_map, (v_pos[0], v_pos[1]), implicit_function
        )
...
class MarchingCube:
    ...
    def get_cube_index(self, condition):
        ...
        for i, point in enumerate(self.corners):
            value = get_func_val_from_map_3d(
                self.function_map,
    ...
    def get_triangles_for_case(self, cube_index):
        ...
        while triangles_to_add[index] != -1:
            edge1, edge2, edge3 = (
                triangles_to_add[index],
                triangles_to_add[index + 1],
                triangles_to_add[index + 2],
            )
    ...
    def get_point_on_edge(self, edge):
        ...
        u_pos, v_pos = self.corners[u], self.corners[v]
        value_u_pos = get_func_val_from_map(
            self.function_map, (u_pos[0], u_pos[1], u_pos[2]), self.implicit_function
        )
        value_v_pos = get_func_val_from_map(
            self.function_map, (v_pos[0], v_pos[1], v_pos[2]), self.implicit_function
        )
...
class MarchingCubesUtils(ThreeDScene):
    ...
    def get_marching_cube(self, key, implicit_function):
        ...
        v_5 = np.array(
            [key[0] + self.x_step, key[1] + self.y_step, key[2] + self.z_step]
        )
...
class TestColorInterpolation(Scene):
    ...
    def calculate_color(self, position, all_positions, colors):
        closest_positions = sorted(
            all_positions, key=lambda x: np.linalg.norm(position - x)
        )[:2]
...
class MetaBalls(MarchingSquaresUtils):
    def construct(self):
        ...
        velocities = [
            UP * i + RIGHT * j
            for i, j in zip(
                np.random.uniform(-1, 1, num_balls), np.random.uniform(-1, 1, num_balls)
            )
        ]
        start_positions = [
            LEFT * 5.5 + UP * 1.5,
        ...
        contour = self.march_squares(
            condition, implicit_function, line_width=5, gradient=True
        )
        marching_squares_end_time = time.time()
        print("Sample space size: {0}".format(len(self.sample_space)))
        print(
            "--- {0} seconds set_sample_space---".format(
                sample_space_end_time - sample_space_start_time
            )
        )
        print(
            "--- {0} seconds get_implicit_function_samples---".format(
                implicit_function_end_time - sample_space_end_time
            )
        )
        print(
            "--- {0} seconds march_squares---".format(
                marching_squares_end_time - implicit_function_end_time
            )
        )
        ...
        def update_ball(balls, dt):
            ...
            new_contour = self.march_squares(
                condition, implicit_function, line_width=5, gradient=True
            )
        ...
        def handle_collision_with_boundary(ball):
            ...
            if ball.get_bottom() <= -VERT_BOUNDARY or ball.get_top() >= VERT_BOUNDARY:
                ball.velocity[1] = -ball.velocity[1]
            # Bounce off left or right
            if (
                ball.get_left_edge() <= -HORIZ_BOUNDARY
                or ball.get_right_edge() >= HORIZ_BOUNDARY
            ):
...
class Introduction(Scene):
    def construct(self):
        ...
        self.play(FadeIn(problem_solving_diagram))
        self.wait()

        self.play(FadeOut(left_rect), problem_solving_diagram.animate.shift(LEFT * 3.7))
        self.wait()

        surround_rect = SurroundingRectangle(
            problem_solving_diagram[:3], buff=SMALL_BUFF * 1.5
        )
        ...
        self.play(Write(left_message_hard))
        self.wait()

        self.play(GrowFromCenter(right_brace))

        self.play(Write(right_message_rewarding))
        self.wait()
    ...
    def get_diagram(self):
        ...
        top_arrow = Arrow(
            vague.get_bottom(), well_defined.get_top(), buff=SMALL_BUFF
        ).set_color(arrow_color)

        bottom_arrow = Arrow(
            well_defined.get_bottom(), solution.get_top(), buff=SMALL_BUFF
        ).set_color(arrow_color)
...
class ShowEasyCases(MarchingSquaresUtils):
    def construct(self):
        ...
        self.play(Write(easy_cases_title), Transform(contour, contour_shifted_down))
        self.wait()
        ...
        self.play(TransformFromCopy(contour, two_circles_case))
        self.play(Create(surround_rect_two_circles))
        two_circles_label = Tex("Two ``circles''").scale(text_scale)
        two_circles_label.next_to(surround_rect_two_circles, DOWN)
        self.play(Write(two_circles_label))
        ...
        self.play(TransformFromCopy(contour, ellipse_case))
        ...
        self.play(TransformFromCopy(contour, circle_case))
        self.wait()
        ...
        self.play(Create(surround_rect_ellipse))
        self.play(Write(ellipse_case_label))
        self.wait()

        self.play(Create(surround_rect_circle))
        ...
        self.play(Write(key_idea_group[0]))
        self.play(Write(key_idea_group[1]))
        self.play(Write(key_idea_group[2]))
        self.wait()
    ...
    def show_and_remove_parameter_labels(self, two_circles_bb, ellipse_bb, circle_bb):
        ...
        left_radius_line = DashedLine(
            center_left_circle, center_left_circle + RIGHT * 0.7
        )
        right_radius_line = DashedLine(
            center_right_circle, center_right_circle + RIGHT * 0.7
        )
        left_radius_label = (
            MathTex("r_0").scale(0.6).next_to(left_radius_line, DOWN, buff=SMALL_BUFF)
        )
        right_radius_label = (
            MathTex("r_1").scale(0.6).next_to(right_radius_line, DOWN, buff=SMALL_BUFF)
        )
        self.play(
        ...
        radius_line = DashedLine(
            center_single_circle, center_single_circle + RIGHT * radius_value
        )
...
class CirclesAndEllipses(Scene):
    ...
    def show_circle_and_ellipse(self):
        ...
        radius_line = DashedLine(
            circle_center_dot.get_center(),
            circle_center_dot.get_center() + normalize(UR) * circle.radius,
        )
        ...
        self.play(GrowFromCenter(circle_center_dot), Write(center_label_circle))
        ...
        a_line = DashedLine(
            ellipse.get_center(), ellipse.get_center() + UP * ellipse.height / 2
        )
        b_line = DashedLine(
            ellipse.get_center(), ellipse.get_center() + RIGHT * ellipse.width / 2
        )
        ...
        self.play(GrowFromCenter(ellipse_center_dot), Write(center_label_ellipse))
        ...
        ellipse_equation = MathTex(
            r"\frac{(x - x_1)^2}{a^2} + \frac{(y - y_1)^2}{b^2} = 1"
        )
        ellipse_equation.add_background_rectangle().move_to(RIGHT * 3.5 + DOWN * 0.7)
        self.play(FadeIn(ellipse_equation))
        self.wait()

        self.play(ellipse_equation.animate.next_to(circle_equation, DOWN))
        self.wait()
        ...
        self.play(FadeIn(implicit_function_def))
        self.wait()

        explicit_function = Tex(r"Explicit function: $y = f(x)$").scale(0.8)
        explicit_function.add_background_rectangle()
        explicit_function.next_to(implicit_function_def, DOWN).shift(
            DOWN * SMALL_BUFF * 2
        )
        self.play(FadeIn(explicit_function))
        self.wait()
...
class ProblemSolvingTechniques(Scene):
    def construct(self):
        ...
        simpler_problem = self.make_component(
            "Simpler" + "\\\\" + "Problem", color=PURE_GREEN
        )
        ...
        simpler_technique = (
            Tex("Ask a simpler version" + "\\\\", "of the problem.")
            .scale(0.8)
            .next_to(arrow_hard_simpler, UP)
        )
        ...
        arrow_hard_harder = Arrow(
            hard_problem_copy.get_right(), harder_problem.get_left()
        )
        arrow_hard_harder.set_color(GRAY)

        harder_technique = (
            Tex("Ask a harder version" + "\\\\", "of the problem.")
            .scale(0.8)
            .next_to(arrow_hard_harder, UP)
        )

        harder_technique[0][4:10].set_color(YELLOW)

        self.animate_component(hard_problem)
        self.play(Write(arrow_hard_simpler))
        self.wait()

        self.play(FadeIn(simpler_technique))
        self.wait()

        self.animate_component(simpler_problem)
        self.play(TransformFromCopy(hard_problem, hard_problem_copy))
        self.wait()

        self.play(Write(arrow_hard_harder))
        self.wait()

        self.play(FadeIn(harder_technique))
        self.wait()
        ...
        hard_problem_summary = (
            ...
            .scale(0.8)
            .next_to(hard_problem_copy, DOWN)
        )

        self.play(FadeIn(hard_problem_summary))
        self.wait()

        transformed_harder_problem = (
            Tex("Find a way to render any" + "\\\\", r"implicit function $f(x, y) = C$")
            .scale(0.8)
            .next_to(harder_problem, DOWN)
        )

        self.play(FadeIn(transformed_harder_problem))
        self.wait()
...
class GuessImplicitFunctionValues(MarchingSquaresUtils):
    ...
    def introduce_sampling_rule(self):
        ...
        self.play(FadeOut(text_group), FadeOut(sample_dot))
        self.wait()
    ...
    def introduce_game(self):
        ...
        self.play(FadeIn(mysterious_function))
        self.wait()
        ...
        i = 0
        for x, y in points:
            label = self.label_point(
                x, y, positive=pos_neg[i], contour_value=contour_mask[i]
            )
            labels.add(label)
            i += 1

        self.play(FadeOut(labels), FadeOut(goal), FadeOut(mysterious_function))
        self.wait()
    ...
    def simplify_game(self):
        ...
        self.play(FadeIn(greater_group), FadeIn(less_group))

        self.wait()

        self.play(FadeOut(greater_group), FadeOut(less_group))
        self.wait()
    ...
    def sample_points_randomly(self, implicit_function):
        ...
        self.play(LaggedStartMap(GrowFromCenter, first_thousand), run_time=2)
        self.wait()

        self.play(LaggedStartMap(GrowFromCenter, to_ten_thousand), run_time=3)
        self.wait()

        self.play(LaggedStartMap(GrowFromCenter, to_twenty_thousand), run_time=3)
        self.wait()
    ...
    def change_blue_point(self, point):
        ...
        left_triangle = Polygon(
            np.array([-2.4, 1, 0]), np.array([-3, 1, 0]), np.array([-3, 2, 0])
        )
        ...
        return (
            general_internal_bb
            or fine_grained_bb_1
            or check_point_inside(point, left_triangle)
            or check_point_inside(point, right_triangle)
            or check_point_inside(point, left_quad)
            or check_point_inside(point, right_quad)
        )
    ...
    def change_green_point(self, point):
        ...
        one_off_quadrants = (
            (x < -2.5 and x > -3 and y > 2 and y < 3)
            or (x > 2.5 and x < 3 and y > 2 and y < 3)
            or (x > -1.1 and x < -0.5 and y < -1.75 and y > -3)
            or (x < 1.1 and x > 0.5 and y < -1.75 and y > -3)
            or (x > 0 and x < 0.5 and y < -2.5 and y > -3)
            or (x > -1 and x < 1 and y > 2.25 and y < 3)
        )
        ...
        return (
            ...
            or check_point_inside(point, left_triangle)
            or check_point_inside(point, right_triangle)
            or check_point_inside(point, top_left_triangle)
            or check_point_inside(point, top_right_triangle)
            or one_off_quadrants
        )
    ...
    def filter_point(self, point):
        ...
        x, y, _ = point
        vertical_and_horizontal_bounds = (
            y < -2.7 or y > 3 or x > -1 and x < 1 and y > -1 and y < 1
        )
        close_y_3 = abs(y - 3) < 0.1
        horiz_line_bottom = (
            x < -4.3 and abs(y + 2.5) < 0.1 or x > 4.3 and abs(y + 2.5) < 0.1
        )
        vert_lines = y < -1.7 and (
            abs(x) < 0.05
            or abs(x + 1) < 0.1
            or abs(x - 1) < 0.1
            or abs(x + 0.85) < 0.05
        ...
        one_off_quadrants = (
            (x > 1 and y > 2 and x < 2 and y < 3)
            or (x < -3 and x > -4 and y > 1 and y < 2)
            or (x < -6.6 and y < -1 and y > -1.5)
            or (y < -1.5 and y > -2.1 and x < -6)
            or (x > 6.52 and y < -1.05 and y > -1.5)
        )
        one_off_points = [
            np.array([-0.8, 1.25, 0]),
            np.array([-2.85, 1.1, 0]),
            np.array([-0.1, -1.8, 0]),
            np.array([-2.85, 2.85, 0]),
            np.array([-5.25, -2.1, 0]),
            np.array([-5.55, -2.05, 0]),
            np.array([-5.8, -2, 0]),
            np.array([-6.45, -1.42, 0]),
            np.array([6.25, -1.5, 0]),
            np.array([6.35, -1.4, 0]),
            np.array([-0.4, -2.35, 0]),
            np.array([0.75, 1.1, 0]),
        ...
        is_close_to_one_off_points = any(
            [np.linalg.norm(point - p) < 0.1 for p in one_off_points]
        )
        other_vert_lines = (
            y < 2
            and y > 0.5
        ...
        return (
            vertical_and_horizontal_bounds
            or close_y_3
            or horiz_line_bottom
            or vert_lines
            or one_off_quadrants
            or is_close_to_one_off_points
            or other_vert_lines
        )
...
class ImplicitFunctionCases(Scene):
    ...
    def show_cases(self):
        ...
        self.play(ApplyWave(contour))
        self.wait()

    def make_note_about_convention(self, f_c):
        self.play(f_c.animate.shift(UP * 2))
        self.wait()

        inside_convention = (
            Tex(r"$f(B) > 1 \Rightarrow $ point $B$ inside contour")
            .to_edge(LEFT * 2)
            .shift(DOWN * 2.2)
        )
        outside_convention = Tex(
            r"$f(A) < 1 \Rightarrow $ point $A$ outside contour"
        ).next_to(inside_convention, DOWN, aligned_edge=LEFT)
        inside_convention[0][-13:-7].set_color(INSIDE_COLOR)
        outside_convention[0][-14:-7].set_color(OUTSIDE_COLOR)

        self.play(FadeIn(inside_convention))
        self.wait()
        self.play(FadeIn(outside_convention))
        self.wait()

        self.play(FadeOut(inside_convention), FadeOut(outside_convention))
        self.wait()
    ...
    def show_key_takeaway(self):
        ...
        self.play(Write(key_takeaway), Write(underline))
    ...
    def show_first_case(self):
        ...
        self.play(DrawBorderThenFill(first_contour))
        self.wait()

        second_contour = self.get_smooth_contour_around_points(A_group, B_group, seed=1)

        self.play(ReplacementTransform(first_contour, second_contour))
        self.wait()

        self.play(FadeOut(second_contour))
    ...
    def get_smooth_contour_around_points(self, A, B, seed=0):
        ...
        A_region_points = [
            A.get_center() + v * np.random.uniform(1, 2.5) for v in directions_A
        ]

        directions_B = [UL, UP, UR, RIGHT, DR, DOWN, DL]

        B_region_points = [
            B.get_center() + v * np.random.uniform(1, 2.5) for v in directions_B
        ]
    ...
    def show_second_case(self, A, B):
        ...
        A_group = (
            VGroup(labelA, pointA, fA).arrange(DOWN).move_to(LEFT * 2.5 + DOWN * 2.5)
        )
        ...
        B_group = (
            VGroup(labelB, pointB, fB).arrange(DOWN).move_to(RIGHT * 2.5 + DOWN * 2)
        )

        self.play(ReplacementTransform(A, A_group), ReplacementTransform(B, B_group))
        self.wait()
    ...
    def show_third_case(self, A, B):
        ...
        B_group = (
            VGroup(labelB, pointB, fB).arrange(DOWN).move_to(RIGHT * 4 + DOWN * 0.5)
        )
    ...
    def show_root_finding(self, A, B):
        ...
        self.play(Create(line_between_points), run_time=2)
        self.wait()
        ...
        self.play(GrowFromCenter(C_dot), Write(C_label))
        ...
        self.play(
            C_dot.animate.move_to(
                line_between_points.point_from_proportion(3 / 4)
        ...
        self.play(
            C_dot.animate.move_to(
                line_between_points.point_from_proportion(5 / 8)
            ).set_color(OUTSIDE_COLOR),
        )
        self.wait()

        self.play(
            C_dot.animate.move_to(
                line_between_points.point_from_proportion(11 / 16)
            ).set_color(OUTSIDE_COLOR),
        )
        self.wait()
        ...
        self.play(
            C_dot.animate.move_to(
                line_between_points.point_from_proportion(23 / 32)
            ).set_color(OUTSIDE_COLOR),
        )
        self.play(
            Flash(C_dot),
...
class FocusOnContour(ZoomedScene, GuessImplicitFunctionValues):
    ...
    def construct(self):
        ...
        self.plane = NumberPlane(
            x_range=(-7.111111111111111, 7.111111111111111, x_step),
            y_range=(-4.0, 4.0, y_step),
        )
        self.add(self.plane)
        self.wait()
    ...
    def zoom_in_on_contour(self):
        ...
        self.wait()
        self.play(frame.animate.shift(RIGHT * 2.5 + DOWN * 0.1), run_time=2)
        self.wait()
        self.play(frame.animate.shift(DOWN * 3.2 + RIGHT * 1), run_time=2)
        self.wait()
        self.play(frame.animate.shift(RIGHT * 2.1 + DOWN * 0.5), run_time=2)
        self.wait()
        self.play(frame.animate.shift(UP * 4.6), run_time=2)
        self.wait()
...
class IntroMarchingSquaresIdea(ZoomedScene, MarchingSquaresUtils):
    ...
    def sample_points_on_grid(self):
        ...
        self.play(LaggedStartMap(GrowFromCenter, sample_dots), run_time=3)
        self.wait()
    ...
    def show_zoomed_cases(self):
        ...
        self.play(FadeIn(outside_contour))
        self.wait()
        ...
        second_dot, previous_rect = self.use_root_finding(
            RIGHT * 0.5 + DOWN * 1,
            RIGHT * 0.5 + DOWN * 1.5,
            RIGHT * 0.75 + DOWN * 1.25,
            previous_rect=previous_rect,
            direction=LEFT,
        )

        estimated_contour_line = Line(
            first_dot.get_center(),
            second_dot.get_center(),
    ...
    def use_root_finding(self, A, B, cell_center, previous_rect=None, direction=UP):
        ...
        if previous_rect:
            self.play(ReplacementTransform(previous_rect, surround_rect))
        else:
            self.play(Create(surround_rect))
        self.wait()
        ...
        dot.move_to(
            line_between_points.point_from_proportion(np.random.uniform(0.2, 0.8))
        )
        self.play(
            Flash(
                dot,
...
class MarchingSquaresCases(Scene):
    ...
    def introduce_cases(self):
        ...
        self.play(FadeIn(all_cases_in_grid))
        self.wait()

        self.highlight_cases_seen(all_cases_in_grid)

        self.set_contour_lines()
        case_10 = self.get_case_contour(
            all_cases_in_grid[1][0], self.get_line_proportions_for_case(1, 0)
        )
        case_12 = self.get_case_contour(
            all_cases_in_grid[1][2], self.get_line_proportions_for_case(1, 2)
        )
    ...
    def organize_cases(self, all_cases_in_grid):
        ...
        self.all_case_polygons[(1, 0)] = self.get_polygon_for_case(
            all_cases_in_grid, 1, 0
        )
        self.all_case_polygons[(1, 2)] = self.get_polygon_for_case(
            all_cases_in_grid, 1, 2
        )
        self.all_case_polygons[(3, 3)] = self.get_polygon_for_case(
            all_cases_in_grid, 3, 3
        )
        ...
        self.play(*[FadeIn(p) for p in self.all_case_polygons.values()])
        self.wait()
        ...
        self.play(
            *[FadeOut(case) for case in to_remove],
            ...
            all_cases_in_grid[0][0].animate.move_to(
                new_group_no_contour[0].get_center()
            ),
            all_cases_in_grid[3][3].animate.move_to(
                new_group_no_contour[1].get_center()
            ),
            self.all_case_polygons[(3, 3)].animate.move_to(
                new_group_no_contour[1].get_center()
            ),
        )
        self.wait()
        ...
        self.show_column(
            all_cases_in_grid, [(1, 0), (2, 0), (0, 1), (0, 2)], LEFT * 2.5
        )

        self.show_column(all_cases_in_grid, [(1, 2), (3, 0), (2, 1), (0, 3)], ORIGIN)

        contours, polygons = self.show_column(
            all_cases_in_grid, [(1, 1), (2, 2)], RIGHT * 2.5
        )
    ...
    def show_column(self, all_cases_in_grid, keys, position):
        ...
        for i, j in keys:
            contours.append(
                self.get_case_contour(
                    all_cases_in_grid[i][j], self.get_line_proportions_for_case(i, j)
                )
            )
            polygons.append(self.get_polygon_for_case(all_cases_in_grid, i, j))
        ...
        self.play(*[FadeIn(p) for p in polygons])
        self.wait()
    ...
    def highlight_edge_cases(self, all_cases_in_grid, contours, polygons):
        ...
        shift_amount = np.linalg.norm(
            edge_cases[0].get_center() - edge_cases[1].get_center()
        )
        ...
        case_1_contour = self.get_case_contour(
            edge_case_1, [(1 / 8, 3 / 8), (5 / 8, 7 / 8)]
        )
        case_2_contour = self.get_case_contour(
            edge_case_2, [(7 / 8, 1 / 8), (3 / 8, 5 / 8)]
        )

        self.animate_showing_case([case_1_contour, case_2_contour])

        polygons_for_case_1 = VGroup(
            *[
                self.get_polygon_for_case(all_cases_in_grid, i, j, case=edge_case_1)
                for i, j in [(4, 0), (4, 1)]
            ]
        )
        polygons_for_case_2 = VGroup(
            *[
                self.get_polygon_for_case(all_cases_in_grid, i, j, case=edge_case_2)
                for i, j in [(4, 2), (4, 3)]
            ]
        )

        self.play(FadeIn(polygons_for_case_1), FadeIn(polygons_for_case_2))
        self.wait()
    ...
    def show_adjustment_of_points(self, case_10, case_12):
        ...
        line10[0].add_updater(
            lambda line: line.become(
                Line(dots10[0].get_center(), dots10[1].get_center()).set_color(
                    CONTOUR_COLOR
                )
            )
        )
        self.add(line10[0])
        line12[0].add_updater(
            lambda line: line.become(
                Line(dots12[0].get_center(), dots12[1].get_center()).set_color(
                    CONTOUR_COLOR
                )
            )
        )
...
class LerpImprovement(Scene):
    ...
    def remind_big_picture(self):
        ...
        self.play(Write(self.plane), run_time=2)
        self.wait()
        ...
        self.play(ReplacementTransform(square, case_square), FadeOut(self.plane))
        self.wait()
    ...
    def show_dual_root_finding(self, case_square):
        ...
        labels = VGroup(
            *[MathTex("f(x_{0}, y_{1})".format(i, j)) for i, j in label_indices]
        )
        ...
        self.wait()
        gt_lt = [">", "<"]
        label_classes = VGroup(
            *[
                MathTex("f(x_{0}, y_{1}) {2} 1".format(i, j, gt_lt[i]))
        ...
        self.play(Write(contour_level))
        self.wait()

        self.play(
            *[
                ReplacementTransform(label, label_class)
                for label, label_class in zip(labels, label_classes)
            ]
        )
        self.wait()
        ...
        for i in range(len(top_proporitions)):
            if i == 0:
                self.play(GrowFromCenter(top_dot), GrowFromCenter(bottom_dot))
                continue
            self.play(
                top_dot.animate.move_to(
                    top_line.point_from_proportion(top_proporitions[i])
                ),
                bottom_dot.animate.move_to(
                    bottom_line.point_from_proportion(bottom_proportions[i])
                ),
            )
            if i == len(top_proporitions) - 1:
                self.play(
                    ...
                    Flash(top_dot),
                    bottom_dot.animate.move_to(
                        bottom_line.point_from_proportion(bottom_proportions[-1])
                    ),
                )

        self.play(bottom_dot.animate.set_color(CONTOUR_COLOR), Flash(bottom_dot))
        self.wait()

        contour_line = Line(top_dot.get_center(), bottom_dot.get_center())
        contour_line.set_color(CONTOUR_COLOR)
        self.play(Create(contour_line))
        self.wait()
    ...
    def motivate_lerp(self, case_square, dots, label_classes):
        ...
        label_values = VGroup(
            *[
                MathTex("f(x_{0}, y_{1}) = {2}".format(i, j, values[j * 2 + i]))
                for i, j in label_indices
            ]
        )
        ...
        self.play(
            *[
                ReplacementTransform(label_class, label_value)
                for label_class, label_value in zip(label_classes, label_values)
            ]
        )
        self.wait()

        self.play(Indicate(label_values[0]))

        self.wait()

        self.play(Indicate(label_values[1]))
        self.wait()
        ...
        self.play(ShowPassingFlash(top_line_highlight, time_width=1))
        self.wait()
        ...
        all_dashed_lines.add(
            dashed_line.copy().move_to(top_line.point_from_proportion(2 / 3))
        )
        all_dashed_lines.add(
            dashed_line.copy().move_to(bottom_line.point_from_proportion(1 / 4))
        )
        all_dashed_lines.add(
            dashed_line.copy().move_to(bottom_line.point_from_proportion(2 / 4))
        )
        all_dashed_lines.add(
            dashed_line.copy().move_to(bottom_line.point_from_proportion(3 / 4))
        )
        self.play(
        ...
        self.wait()
        top_dot = (
            Dot().set_color(CONTOUR_COLOR).move_to(all_dashed_lines[0].get_center())
        )
        approx_top = MathTex(r"f(x, y) \approx 1").scale(0.7)
        approx_top.next_to(top_dot, DOWN)
        self.play(Write(approx_top))

        self.play(GrowFromCenter(top_dot), Flash(top_dot))
        self.wait()

        self.play(ShowPassingFlash(bottom_line_highlight, time_width=1))
        self.wait()
        ...
        self.play(Write(approx_bottom))
        self.play(GrowFromCenter(bottom_dot), Flash(bottom_dot))
        self.wait()
        ...
        self.play(
            ...
            label_values[0]
            .animate.shift(shift_amount + UP * SMALL_BUFF / 2)
            .scale(0.8),
            label_values[1]
            .animate.shift(shift_amount + UP * SMALL_BUFF / 2)
            .scale(0.8),
            label_values[2]
            .animate.shift(shift_amount + DOWN * SMALL_BUFF / 2)
            .scale(0.8),
            label_values[3]
            .animate.shift(shift_amount + DOWN * SMALL_BUFF / 2)
            .scale(0.8),
        ...
        left_dot, right_dot = Dot().set_color(INSIDE_COLOR), Dot().set_color(
            OUTSIDE_COLOR
        )
        ...
        self.play(Write(result_y))
        self.wait()

        observe_x = MathTex(
            r"\frac{x - x_0}{x_1 - x_0}",
            r" \approx ",
            r"\frac{f(x, y) - f(x_0, y_0)}{f(x_1, y_0) - f(x_0, y_0)}",
        )
        observe_x.scale(0.9)
        observe_x.next_to(result_y, DOWN)

        self.play(FadeIn(observe_x))
        self.wait()
        ...
        new_observe_x = MathTex(
            r"\frac{x - x_0}{x_1 - x_0}", r" \approx ", r" \frac{1 - v_0}{v_1 - v_0}"
        )
...
class MarchingSquaresSummary(MarchingSquaresUtils):
    ...
    def show_marching_of_squares(self):
        ...
        contour = self.march_squares(
            condition, implicit_function, value=value, line_width=5
        )
        marching_squares_end_time = time.time()
        print("Sample space size: {0}".format(len(self.sample_space)))
        print(
        ...
        self.play(GrowFromCenter(brace_contour))
        isocontour = Tex("Isocontour").set_color(CONTOUR_COLOR)
        isocontour.next_to(brace_contour, RIGHT)
        self.play(Create(surround_rect), Write(isocontour))
        self.wait()

        new_title = Tex(
            r"Marching Squares $\rightarrow$ extracts isocontours from implicit functions"
        ).scale(0.8)
        new_title.next_to(title[1], UP)
        new_title[0][24:35].set_color(CONTOUR_COLOR)
        self.play(Transform(title[0], new_title[0]))
        self.wait()
        ...
        for key in self.function_map:
            ...
            # self.add(case_integer)
            lines = self.process_case(
                case, marching_square, implicit_function, value=value, width=5
            )
            if current_square:
                self.remove(current_square)
            current_square = self.get_square_geom_from_corners(
                square_corners, implicit_function, value=value
            )
    ...
    def get_square_geom_from_corners(self, corners, implicit_function, value=1):
        ...
        for corner in corners:
            sample_value = get_func_val_from_map(
                self.function_map, (corner[0], corner[1]), implicit_function
            )
...
class IntroParallelization(Scene):
    def construct(self):
        ...
        self.play(Write(embarrassingly_parallel[0]))
        self.wait()

        defintion = Tex("- Easily able to be separated into independent tasks").scale(
            0.8
        )
        ...
        speed_up = Tex(
            r"Parallilized Marching Squares", r" $\rightarrow$ ", "improved performance"
        ).scale(0.9)

        speed_up.next_to(defintion, DOWN * 2)

        self.play(Write(speed_up[0]))
        self.wait()

        self.play(Write(speed_up[1]))
        self.play(Write(speed_up[2]))

        self.wait()

        side_length = 1.6
        grid = VGroup(
            *[Square(side_length=side_length).set_color(BLUE_D) for _ in range(9)]
        )
        ...
        self.play(LaggedStartMap(GrowFromCenter, computers))
        self.wait()
        ...
        self.play(Write(shared_function_map))
        self.wait()
        ...
        self.play(arrow.animate.shift(UP * side_length / 2), Write(second_arrow))
        self.wait()
        ...
        self.play(Write(shared_write_only))
        self.wait()
        ...
        for i in range(len(contour_points)):
            prev, current = (
                contour_points[i],
                contour_points[(i + 1) % len(contour_points)],
            )
            contour_line = Line(prev, current).set_color(CONTOUR_COLOR)
            contour_lines.add(contour_line)

        self.play(*[Create(line) for line in contour_lines])
        self.wait()
...
class StepBack(Scene):
    def construct(self):
        ...
        self.play(Create(screen_rectangle))
        self.wait()
        ...
        self.play(ReplacementTransform(second, third))
        self.wait()

        self.play(ReplacementTransform(third, fourth))
        self.wait()

        fifth = Tex(
            "I could tell you ... ", "but we can actually re-discover it!"
        ).scale(1)

        self.play(FadeOut(fourth))
        self.wait()

        fifth.next_to(screen_rectangle, DOWN)

        self.play(Write(fifth[0]))
        self.wait()

        self.play(Write(fifth[1]))
        self.wait()
...
class EnergyFieldsTakeTwoCut(MarchingSquaresUtils):
    def construct(self):
        ...
        contour_function_values = [
            MathTex(f"f(x, y) = {value}") for value in contour_values
        ]
        ...
        contour_g_function_values = [
            MathTex(f"g(x, y) = {value}") for value in contour_values
        ]
        ...
        surround_rect = SurroundingRectangle(
            VGroup(dual_energy_fields, f_xy, g_xy), buff=MED_SMALL_BUFF
        )
        bigger_rect = Rectangle(
            height=surround_rect.height + 0.2, width=surround_rect.width + 2.5
        )
        ...
        self.play(Write(bigger_rect))
        self.play(Write(sum_implicit_functions))
        self.wait()

        self.show_metaball_implicit_functions(
            f_xy,
            g_xy,
            bigger_rect,
            sum_implicit_functions,
            metaballs,
            dual_energy_fields,
            sum_implicit_functions,
        )
        ...
        vector_field = self.get_vector_field(
            metaballs[1].get_center(), metaballs[0].get_center(), velocity
        )
        left_proton = get_proton().move_to(metaballs[1].get_center())
        right_proton = get_proton().move_to(metaballs[0].get_center())
        self.play(GrowFromCenter(left_proton), GrowFromCenter(right_proton))
        self.play(LaggedStartMap(Create, vector_field), run_time=2)
        self.wait()
        ...
        def second_updater_with_vector_field(metaballs, dt):
            ...
            new_vector_field = self.get_vector_field(
                metaballs[1].get_center(), metaballs[0].get_center(), velocity
            )
        ...
        self.show_electric_potential_connection(
            left_proton, right_proton, dual_energy_fields, vector_field
        )

    def show_metaball_implicit_functions(
        self,
        f_xy,
        g_xy,
        bigger_rect,
        ...
        self.play(GrowFromCenter(contour_dot), Write(x_y))
        self.wait()
        ...
        self.play(Write(f_result), Write(g_result))
        self.wait()

        f_result_transformed = MathTex(
            r"= \frac{1}{\sqrt{(x - x_1) ^ 2 + (y - y_1) ^ 2}}"
        ).scale(0.8)
        g_result_transformed = MathTex(
            r"= \frac{1}{\sqrt{(x - x_2) ^ 2 + (y - y_2) ^ 2}}"
        ).scale(0.8)
        ...
        self.play(ShowPassingFlash(underline, time_width=2))
        self.wait()
    ...
    def show_electric_potential_connection(
        self, left_proton, right_proton, dual_energy_fields, vector_field
    ):
        ...
        self.play(FadeIn(electric_potential_title))
        ...
        label.move_to(potential_point).shift(
            normalize(potential_point) * SMALL_BUFF * 5
        )
        ...
        self.play(GrowFromCenter(dot), FadeIn(P_label))
        self.wait()
        self.play(FadeIn(label))
        ...
        self.play(FadeIn(r_1), FadeIn(r_2))
        self.wait()
        ...
        self.play(FadeIn(V_Q1), FadeIn(V_Q2))
        self.wait()
    ...
    def get_vector_field(self, left_center, right_center, velocity):
        ...
        vector_field = ArrowVectorField(
            field_function_two_charges(
                left_center + left_shift, right_center + right_shift
            )
        )
        return vector_field
...
class ShowMetaballAdjustments(MarchingSquaresUtils):
    ...
    def show_position_and_scale_adjustment(self):
        ...
        (
            ...
            second_equation,
            summed_equation,
        ) = self.get_implicit_function_of_metaballs()
        ...
        self.play(GrowFromCenter(left_center_dot), FadeIn(left_center_label))
        self.wait()

        first_equation.move_to(UP * 3)
        self.play(FadeIn(first_equation))
        self.wait()

        radius_line = Line(
            left_center_dot.get_center(), left_center_dot.get_center() + normalize(UR)
        )
        ...
        self.play(ReplacementTransform(first_equation, second_equation))
        self.wait()
        ...
        def update_metaballs_scale(metaballs, dt):
            ...
            contour.become(new_contour)
            new_line = Line(
                left_center_dot.get_center(),
                left_center_dot.get_center() + normalize(UR) * metaballs[0].radius,
            )
        ...
        f_always(
            ...
            left_center_dot.get_center,
            lambda: left_center_dot.get_center() + normalize(UR),
        )
        f_always(
            radius_label.move_to, lambda: radius_line.get_center() + UL * SMALL_BUFF * 3
        )
        self.wait(2)
        ...
        metaballs = VGroup(right_ball)
        self.remove(
            contour, radius_line, radius_label, left_center_dot, left_center_label
        )
        ...
        right_equation = (
            MathTex(r"\frac{R_1}{\sqrt{(x - x_1) ^ 2 + (y - y_1) ^ 2}} = 1")
            .scale(1)
            .add_background_rectangle()
        )
        right_equation.move_to(UP * 3)
        right_radius_line = Line(
            right_center_dot.get_center(), right_center_dot.get_center() + normalize(UR)
        )
        right_radius_label = MathTex("R_1").scale(0.8).add_background_rectangle()

        right_radius_label.move_to(right_radius_line.get_center()).shift(
            UL * SMALL_BUFF * 3
        )
    ...
    def get_implicit_function_of_metaballs(self, scale=1):
        first_equation = (
            MathTex(r"\frac{1}{\sqrt{(x - x_0) ^ 2 + (y - y_0) ^ 2}} = 1")
            .scale(scale)
            .add_background_rectangle()
        )
        second_equation = (
            MathTex(r"\frac{R_0}{\sqrt{(x - x_0) ^ 2 + (y - y_0) ^ 2}} = 1")
            .scale(scale)
            .add_background_rectangle()
        )

        summed_equation = MathTex(
            r"\frac{R_0}{\sqrt{(x - x_0) ^ 2 + (y - y_0) ^ 2}} + \frac{R_1}{\sqrt{(x - x_1) ^ 2 + (y - y_1) ^ 2}} = 1"
        ).scale(scale)
...
class MetaBallsWithLabels(MarchingSquaresUtils):
    def construct(self):
        ...
        radius_lines = [
            DashedLine(ball.get_center(), ball.get_center() + ball.radius * UP)
            for ball in balls
        ]
        radius_labels = [
            MathTex("R_{0}".format(i + 1)).scale(0.7).next_to(line, RIGHT)
            for i, line in enumerate(radius_lines)
        ]
        dots = [Dot().move_to(ball.get_center()) for ball in balls]
        dot_labels = [
            MathTex("(x_{0}, y_{1})".format(i + 1, i + 1))
            .scale(0.7)
        ...
        implicit_function = MathTex(
            r"\sum_{i=1}^{8} \frac{R_i}{\sqrt{(x - x_i) ^ 2 + (y - y_i) ^ 2}} = C"
        )
        implicit_function.to_edge(RIGHT).shift(DOWN * 3)

        self.play(Write(implicit_function))
        self.wait()
...
class OverlayMarchingCubes(Scene):
    ...
    def make_text_overlays(self):
        ...
        self.play(Write(first_resolution))

        self.wait()

        self.play(ReplacementTransform(first_resolution, second_resolution))
        self.wait()
        ...
        self.play(FadeIn(video_title), FadeIn(author))
        self.wait()
...
class TransitionToConclusion(Scene):
    def construct(self):
        ...
        flow_of_ideas = VGroup(
            info_theory, down_arrow, data_compression, down_arrow.copy(), huffman_codes
        ).arrange(DOWN)
        ...
        for i in range(1, len(flow_of_ideas)):
            if i % 2 == 0:
                self.play(FadeIn(flow_of_ideas[i]))
            else:
                self.play(Write(flow_of_ideas[i]))
            self.wait()
...
class Conclusion(Scene):
    def construct(self):
        ...
        self.play(Write(title))
        self.wait()
        self.play(FadeIn(first_encounter))
        self.wait()
        ...
        self.play(FadeIn(this_video))
        self.wait()

        build_component = self.make_component("Build" + "\\\\" + " metaballs")
        marching_squares_component = self.make_component(
            "Marching" + "\\\\" + "squares/cubes", color=ORANGE
        )
        implement_component = self.make_component(
            "Implement" + "\\\\" + "algorithms", color=RED
        )
        ...
        self.play(ReplacementTransform(hard, rewarding))
        self.wait()
...
class MetaBallsThumbnail(MarchingSquaresUtils):
    def construct(self):
        ...
        implicit_function_end_time = time.time()
        condition = lambda x: x >= 1
        white_contour = self.march_squares(
            condition, implicit_function, line_width=12, color=YELLOW
        )
        contour = self.march_squares(
            condition, implicit_function, line_width=2, gradient=True
        )
        polygon_fill = self.march_squares(
            condition, implicit_function, line_width=5, gradient=True, fill=True
        )
        marching_squares_end_time = time.time()
        print("Sample space size: {0}".format(len(self.sample_space)))
        print(
...
def get_random_point_in_frame(buff=0):
    x_min, x_max = (
        floor(-config.frame_width / 2) + buff,
        floor(config.frame_width / 2) - buff,
    )
    y_min, y_max = (
        floor(-config.frame_height / 2) + buff,
        floor(config.frame_height / 2) - buff,
    )
    return np.array(
        [np.random.uniform(x_min, x_max), np.random.uniform(y_min, y_max), 0]
    )
...
def batman_function(sample_point):
    ...
    eq1 = (
        ...
        + (y / 3) ** 2 * sqrt(abs(y + 3 / 7 * sqrt(33)) / (y + 3 / 7 * sqrt(33)))
        - 1
    )
    eq2 = (
        abs(x / 2)
    ...
    eq3 = (
        9 * sqrt(abs((abs(x) - 1) * (abs(x) - 0.75)) / ((1 - abs(x)) * (abs(x) - 0.75)))
        - 8 * abs(x)
        - y
    )
    eq4 = (
...
def implicit_function_group_of_circles(circles):
    def func(sample_point):
        ...
        return sum(
            [implicit_function_circle(sample_point, circle) for circle in circles]
        )
...
def implicit_function_group_of_circles_sqrt(circles):
    def func(sample_point):
        ...
        # return sum([test_function(sample_point, circle) for circle in circles])
        return sum(
            [implicit_function_circle_sqrt(sample_point, circle) for circle in circles]
        )
...
def get_computer_mob():
    ...
    base = Rectangle(
        height=outer_rect.get_height() - inner_rect.get_height(),
        width=outer_rect.get_width() * 0.7,
    )
    base.set_color(GRAY_D).set_fill(color=GRAY_D, opacity=1)
    connector = Rectangle(
        height=base.get_width() * 0.3 * 0.8, width=base.get_width() * 0.3
    )
