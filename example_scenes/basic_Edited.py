...
class OpeningManim(Scene):
    def construct(self):
        title = Tex(r"\LaTeX: N. Djafar Henni")
        ...
        transformed_title = Tex("That was a transform")
        transformed_title.to_corner(UP + LEFT)
        self.play(
            Transform(title, transformed_title),
            LaggedStart(*(FadeOut(obj, shift=DOWN) for obj in basel)),
        )
        self.wait()

        grid = NumberPlane().scale(0.5)
        grid_title = Tex("This is a grid", font_size=72)
        grid_title.move_to(transformed_title) #Move the center of grid_title text to the coordinates of 
        #transformed title

        self.add(grid, grid_title)  # Make sure title is on top of grid (grid added 1st, then grid_title 2nd)
                                    #so the title is on the grid
                                    #/!\ Order is important, read definition of self.add() 
        self.play(
            FadeOut(title),
            FadeIn(grid_title, shift=UP),
            Create(grid, run_time=3, lag_ratio=1),
        )
        self.wait()
