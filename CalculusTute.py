...
class CalculusArea(Scene):
    def construct(self):
        ...
        for k in range(1, len(dx_list)):
            ...
            self.play(
                Transform(first_area, new_area), run_time=3
            )  # It should be first_area it's the original one
            # if you write first_area[k-1] it'll copy the object.
            self.wait(0.5)

        self.wait()


class VectorFields(Scene):  # My note:    Doesn't work because of p[] and not p
    def construct(self):
